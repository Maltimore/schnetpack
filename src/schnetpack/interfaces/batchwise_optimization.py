...
class BatchwiseCalculator:
    ...
    def _load_model(self, model: str) -> nn.Module:
        return torch.load(model, map_location="cpu", weights_only=False).to(torch.float64)
...
class BatchwiseEnsembleCalculator(BatchwiseCalculator):
    ...
    def _load_model(self, model: str) -> nn.ModuleList:
        ...
        for m_path in model_paths:
            m = torch.load(os.path.join(m_path, "best_model"), map_location="cpu", weights_only=False).to(
                torch.float64
            )
            models.append(m)
